...
async def send_daily_summary():
    ...
    try:
        ...
        console_out(f"❌ Daily Summary Error: {e}")
...
async def generate_content(prompt):
    ...
    if not MODEL_POOL:
        console_out("⚠️ MODEL_POOL empty, restoring defaults...")
    ...
    if not API_KEY_POOL:
        console_out("⚠️ API_KEY_POOL empty!")
        return "❌ No API keys available", "❌ No API keys available", "CRITICAL_ERROR"
        ...
        while temp_model_idx < max_models:
            ...
            try:
                ...
                try:
                    await bot.send_message(
                        OWNER_ID,
                        f"⚠️ <b>{fail_reason.upper()}: {model_id}</b>\n"
    ...
    if not MODEL_POOL:
        await bot.send_message(OWNER_ID, "⚠️ <b>ALL MODELS REMOVED/FAILED. RESTORING DEFAULTS...</b>", parse_mode=ParseMode.HTML)
...
async def cmd_start(message: types.Message, state: FSMContext):
    ...
    await message.answer(f"💎 <b>SINGULARITY V5.0 LIVE | {MODEL_POOL[CURRENT_MODEL_INDEX]}</b>\nSystems Online.", reply_markup=kb, parse_mode=ParseMode.HTML)
    ...
    if "Error" in c_type or "CRITICAL" in c_type or public_content.startswith("Error:"):
        ...
        try:
            ...
            try:
                ...
                await cb.message.answer(
                    f"✅ BREACH READY (Preview too complex to display)\n\nClick FIRE to deploy.\n\nℹ️ {str(e)[:100]}",
                    reply_markup=kb
                )
...
async def topic_res(message: types.Message, state: FSMContext):
    ...
    if "Error" in c_type or "CRITICAL" in c_type:
         ...
         await message.answer(f"❌ <b>GENERATION FAILED</b>\n\n{public_content}", reply_markup=kb, parse_mode=ParseMode.HTML)
    else:
        kb = InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="🔥 FIRE TO CHANNEL", callback_data="fire")],
            [InlineKeyboardButton(text="🔙 BACK", callback_data="back_main")]
...
async def fire_exec(cb: types.CallbackQuery, state: FSMContext):
    ...
    if "Error" in data.get('c_type', "") or "CRITICAL" in data.get('c_type', ""):
        await cb.answer("❌ BLOCKED: Content contains errors.", show_alert=True)
    ...
    try:
        ...
        await cb.message.edit_text(f"❌ Failed to post to channel: {e}")
    ...
    try:
        ...
        console_out(f"✅ Breach forwarded to Vault Channel (ID hidden).")
    except Exception as e:
        console_out(f"⚠️ Vault forward failed: {e}")
    
    # Mark as fired for duplicate detection
    mark_as_fired(public_content)
    DAILY_STATS["breaches_fired"] += 1
    
    await cb.message.edit_text("🚀 DEPLOYED TO CHANNEL & VAULT.\n(Breach ID visible only to you)")
    await state.clear()
...
async def back_main(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.answer("💎 MAIN MENU", reply_markup=ReplyKeyboardMarkup(keyboard=[
        [KeyboardButton(text="🔥 BREACH"), KeyboardButton(text="🗓 SCHEDULE")],
        [KeyboardButton(text="⚙️ MODELS"), KeyboardButton(text="🔑 API")],
        [KeyboardButton(text="🛡 SCAN"), KeyboardButton(text="📢 BROADCAST")]
    ], resize_keyboard=True))
...
async def t60_preflight(job_id, fire_time):
    ...
    cap_warning = ""
    if usage >= (limit * 0.9):
        cap_warning = f"\n⚠️ <b>CAPACITY CRITICAL:</b> {usage}/{limit} RPD used."
...
async def sched_time(message: types.Message, state: FSMContext):
    ...
    # Handle DONE
    if txt == "✅ DONE (NEXT STEP)":
        data = await state.get_data()
        if not data.get("timings"):
            await message.answer("⚠️ Please add at least one time first.")
            return
...
async def sched_month(message: types.Message, state: FSMContext):
    ...
    if not val.isdigit() or not (1 <= int(val) <= 12):
        await message.answer("❌ Invalid Month. Enter 1-12:", reply_markup=kb)
        return
...
async def sched_year(message: types.Message, state: FSMContext):
    ...
    if "PERPETUAL" in val or "INFINITE" in val:
        ...
        await message.answer("❌ Invalid Year. Format: YYYY", reply_markup=kb)
        return
    else:
...
async def cmd_gatekeeper_menu(message: types.Message):
    status_icon = "✅ ON" if GATEKEEPER_ENABLED else "❌ OFF"
    kb = InlineKeyboardMarkup(inline_keyboard=[[
...
async def lock_sched(cb: types.CallbackQuery, state: FSMContext):
    ...
    if recommendation:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="⚠️ FORCE UNSAFE LOCK", callback_data="sched_force")]
        ])
        await cb.message.edit_text(text, reply_markup=kb, parse_mode=ParseMode.HTML)
    else:
        # No better option, warning only
        text = (
            f"⚠️ <b>CRITICAL CAPACITY WARNING</b>\n"
            f"━━━━━━━━━━━━━━━━━━━━\n"
            f"Required: {daily_load} | Limit: {current_limit}\n"
            f"No available model can safely handle this load."
        )
        kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text="⚠️ FORCE LOCK", callback_data="sched_force")]])
        await cb.message.edit_text(text, reply_markup=kb, parse_mode=ParseMode.HTML)
...
async def sched_decision(cb: types.CallbackQuery, state: FSMContext):
    ...
    if "switch" in action:
        ...
        if target in MODEL_POOL:
            ...
            await cb.answer(f"✅ Switched to {target}")
...
async def execute_lock(message, state, data):
    ...
    limit = MODEL_SPECS.get(model_id, {}).get("rpd", 1500)
    daily_load = len(timings)
    status = "✅ SAFE" if daily_load <= limit else "⚠️ UNSAFE"
    ...
    breach_str = f"🚫 <b>{total_breaches:,}</b>" if total_breaches > 0 else "✅ 0"
...
async def model_menu(message: types.Message, state: FSMContext):
    ...
    if not MODEL_POOL:
        dash.append("⚠️ <i>NO MODELS DETECTED. PLEASE ADD ONE.</i>")
    else:
...
async def model_select(message: types.Message, state: FSMContext):
    ...
    if "REMOVE MODEL" in txt:
        if not MODEL_POOL:
            await message.answer("⚠️ No models to remove.")
            return
            
        kb_list = []
        for i, m in enumerate(MODEL_POOL):
            kb_list.append([InlineKeyboardButton(text=f"🗑️ {m}", callback_data=f"del_model_{i}")])
        kb_list.append([InlineKeyboardButton(text="❌ DONE", callback_data="del_model_cancel")])
    ...
    if txt.isdigit():
        ...
        if 0 <= idx < len(MODEL_POOL):
            ...
            await message.answer(f"✅ <b>ENGINE SWITCHED:</b> {MODEL_POOL[idx]}\n💾 Saved to database.", parse_mode=ParseMode.HTML)
            # Re-show menu
    ...
    if txt in MODEL_POOL:
        ...
        await message.answer(f"✅ <b>ENGINE ACTIVE:</b> {txt}", parse_mode=ParseMode.HTML)
        # Re-show menu
        await model_menu(message, state)
    else:
        await message.answer("❌ Invalid Engine. Select from menu or type Index.")
...
async def model_add_save(message: types.Message, state: FSMContext):
    ...
    if added_count > 0:
        ...
        await message.answer(
            f"✅ <b>{added_count} MODEL(S) ADDED & NORMALIZED</b>\n\n{preview}",
            parse_mode=ParseMode.HTML
        )
    else:
        await message.answer("⚠️ No new models found or duplicates ignored.")
...
async def api_menu(message: types.Message, state: FSMContext):
    ...
    for i, k in enumerate(API_KEY_POOL):
        ...
        marker = "✅" if i == CURRENT_API_INDEX else "🔹"
...
async def api_select(message: types.Message, state: FSMContext):
    ...
    if "REMOVE KEY" in txt:
        if not API_KEY_POOL:
            await message.answer("⚠️ No keys to remove.")
            return
        ...
        kb_list.append([InlineKeyboardButton(text="❌ DONE", callback_data="del_key_cancel")])
    ...
    if 0 <= idx < len(API_KEY_POOL):
        ...
        try:
            ...
            await message.answer(f"✅ <b>KEY SWITCHED:</b> Key {idx+1}", parse_mode=ParseMode.HTML)
        except Exception as e:
            await message.answer(f"⚠️ <b>KEY ERROR:</b> {e}", parse_mode=ParseMode.HTML)
        
        await api_menu(message, state)
        return
    else:
         await message.answer("❌ Invalid Key Index.", parse_mode=ParseMode.HTML)
...
async def api_add_save(message: types.Message, state: FSMContext):
    ...
    if added_count > 0:
        ...
        await message.answer(f"✅ <b>{added_count} KEY(S) ADDED.</b>", parse_mode=ParseMode.HTML)
    else:
        await message.answer("⚠️ No new keys found or duplicates ignored.")
...
@dp.message(F.text == "✅ CONFIRM DELETE")
async def history_del_confirm_reply(message: types.Message, state: FSMContext):
    ...
    if docs:
        ...
        status = "✅ <b>HISTORY CLEARED.</b> Items moved to Recycle Bin."
    else:
        status = "⚠️ History is empty."
    ...
    kb = ReplyKeyboardMarkup(keyboard=[
        ...
        [KeyboardButton(text="🛡 SCAN"), KeyboardButton(text="📢 BROADCAST")],
        [KeyboardButton(text="📜 HISTORY"), KeyboardButton(text="🗑️ RECYCLE BIN")],
        [KeyboardButton(text="🛡️ GATEKEEPER"), KeyboardButton(text="❌ DELETE ALL")]
    ], resize_keyboard=True)
...
async def bin_restore_reply(message: types.Message):
    ...
    if docs:
        ...
        col_bin.delete_many({})
        await message.answer("✅ <b>RESTORE SUCCESSFUL.</b>\nAll items moved back to History.", parse_mode=ParseMode.HTML)
        await show_bin_page(message, 0)
    else:
        await message.answer("⚠️ Bin is empty.")
...
async def bin_purge_reply(message: types.Message):
    ...
    if count > 0:
        ...
        await message.answer("⚠️ Bin is already empty.")
...
async def broad_exec(message: types.Message, state: FSMContext):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="❌ ABORT", callback_data="broad_cancel")]
    ])
...
async def fire_exec(cb: types.CallbackQuery, state: FSMContext):
    ...
    await cb.message.edit_text("⏳ <b>ACCESSING NEURAL NETWORK...</b>", parse_mode=ParseMode.HTML)
    ...
    if GATEKEEPER_ENABLED:
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[[
            InlineKeyboardButton(text="🚀 APPROVE & PUBLISH", callback_data="gate_approve"),
            InlineKeyboardButton(text="❌ DISCARD", callback_data="gate_discard")
        ]])
        ...
        await cb.message.edit_text(f"✅ <b>CONTENT GENERATED & SENT FOR REVIEW.</b>\nCheck Admin DM.", parse_mode=ParseMode.HTML)
        
    else:
        # AUTONOMOUS MODE
        try:
            sent = await bot.send_message(CHANNEL_ID, content, parse_mode=ParseMode.HTML)
            await cb.message.edit_text(f"🚀 <b>DEPLOYED TO CHANNEL.</b>\nID: {sent.message_id}", parse_mode=ParseMode.HTML)
        except Exception as e:
             await cb.message.edit_text(f"⚠️ <b>DEPLOY FAIL:</b> {e}", parse_mode=ParseMode.HTML)
...
async def t0_execution(job_id):
    ...
    if status == "CRITICAL_ERROR":
        await bot.send_message(OWNER_ID, f"⚠️ SCHEDULE FAILED: {content}")
        return
...
async def gate_approve_handler(cb: types.CallbackQuery):
    ...
    try:
        ...
        await cb.message.edit_text(f"✅ <b>APPROVED & PUBLISHED.</b>\n\n{final_content[:50]}...", parse_mode=ParseMode.HTML)
    except Exception as e:
        await cb.message.answer(f"⚠️ Publish Error: {e}")

@dp.callback_query(F.data == "gate_discard")
async def gate_discard_handler(cb: types.CallbackQuery):
    await cb.message.edit_text("❌ <b>CONTENT DISCARDED by Admin.</b>", parse_mode=ParseMode.HTML)
...
async def manual_fire_confirm(cb: types.CallbackQuery):
    ...
    if job_id in PENDING_FIRE and not PENDING_FIRE[job_id]["fired"]:
        ...
        PENDING_FIRE[job_id]["fired"] = True
        await cb.message.edit_text("🚀 MANUAL FIRE SUCCESSFUL.")
    await cb.answer()
...
async def main():
    ...
    try:
        if col_system is not None:
             ...
             if conf:
                 ...
                 if doc_models and "list" in doc_models and doc_models["list"]:
                     ...
                     console_out(f"✅ MODELS LOADED FROM DB: {len(MODEL_POOL)} models")
                     ...
                     console_out(f"⚠️ No models in DB, initialized with {len(MODEL_POOL)} default models")
                 ...
                 if doc_keys and "list" in doc_keys and doc_keys["list"]:
                     ...
                     console_out(f"✅ API KEYS LOADED FROM DB: {len(API_KEY_POOL)} keys")
                 else:
                     ...
                     if old_conf and "api_keys" in old_conf and old_conf["api_keys"]:
                         ...
                         console_out(f"✅ MIGRATED {len(API_KEY_POOL)} API KEYS FROM OLD CONFIG")
                     elif GEMINI_KEY:
                         ...
                         console_out(f"⚠️ No API keys in DB, initialized with 1 key from .env")
                     else:
                 ...
                 if API_KEY_POOL:
                     ...
                     try: 
                         client = genai.Client(api_key=GEMINI_KEY)
                         console_out(f"✅ Gemini client initialized with Key 1/{len(API_KEY_POOL)}")
                     except Exception as e:
                         console_out(f"⚠️ Failed to init client: {e}")
                 ...
                 if conf.get("current_model_index") is not None:
                     ...
                     if 0 <= saved_idx < len(MODEL_POOL):
                         ...
                         console_out(f"✅ Restored model index: {CURRENT_MODEL_INDEX} ({MODEL_POOL[CURRENT_MODEL_INDEX]})")
                     else:
                         console_out(f"⚠️ Saved index {saved_idx} out of range, using default")
                 else:
        ...
        # Keep port open for Render even if bot fails
